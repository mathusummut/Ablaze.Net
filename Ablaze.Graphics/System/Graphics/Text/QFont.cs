...
namespace System.Graphics.Text {
	...
	public class QFont : IDisposable {
		...
		public QFont(string fontPath, float size, QFontBuilderConfiguration config,
			...
			RectangleF? transToVp = null;
			float fontScale = 1f;
			if (config.TransformToCurrentOrthogProjection)
				transToVp = OrthogonalTransform(out fontScale, ref currentProjectionMatrix);
		...
		private void InitialiseGlFont(Font font, QFontBuilderConfiguration config, QFontData data = null) {
			_fontData = data == null ? BuildFont(font, config, null) : data;
		...
		/// <param name="orthoProjMatrix"></param>
		private static RectangleF OrthogonalTransform(out float fontScale, ref Matrix4 orthoProjMatrix) {
			if (!ViewportHelper.IsOrthographicProjection(ref orthoProjMatrix))
				throw new ArgumentOutOfRangeException(nameof(orthoProjMatrix),
			...
			RectangleF viewportTransform = ViewportHelper.GetViewportFromOrthographicProjection(ref orthoProjMatrix);
