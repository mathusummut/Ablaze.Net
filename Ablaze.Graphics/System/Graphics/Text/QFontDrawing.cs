...
void main(void)
{
	...
	gl_Position = proj_matrix * vec4(in_position, 1.); 
		...
		private readonly List<QFontDrawingPrimitive> _glFontDrawingPimitives;
		...
		public QFontDrawing(bool useDefaultBlendFunction = true) {
			...
			_glFontDrawingPimitives = new List<QFontDrawingPrimitive>();
			InitialiseState();
		}

		public static SharedState QFontSharedState {
			get {
				return _QFontSharedState;
			}
		}

		public SharedState InstanceSharedState {
			get {
				return QFontSharedState;
			}
		}
		...
		public List<QFontDrawingPrimitive> DrawingPimitiveses {
			get {
				return _glFontDrawingPimitives;
			}
		}
		...
		private static void InitialiseStaticState() {
			...
			GL.GetProgram(prog, ProgramParameter.LinkStatus, out progLinkStatus);
			if (progLinkStatus == 0) {
		...
		public void Draw() {
			...
			foreach (var primitive in _glFontDrawingPimitives) {
				const BeginMode dpt = BeginMode.Triangles;
		...
		public void RefreshBuffers() {
			if (vertexArrayObject == null) {
				vertexArrayObject = new QVertexArrayObject(QFontSharedState);
			}
	...
	public class SharedState {
		...
		public TextureUnit DefaultTextureUnit {
			get; private set;
		}
