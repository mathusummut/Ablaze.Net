...
namespace System.Graphics.Text {
	public class QFontDrawingPrimitive {
		...
		public float LineSpacing {
			get {
				return (float) Math.Ceiling(Font.FontData.maxGlyphHeight * this.Options.LineSpacing);
			}
		}

		public bool IsMonospacingActive {
			get {
				return Font.FontData.IsMonospacingActive(this.Options);
			}
		}

		public float MonoSpaceWidth {
			get {
				return Font.FontData.GetMonoSpaceWidth(this.Options);
			}
		}
		...
		internal IList<QVertex> CurrentVertexRepr {
			get {
				return _currentVertexRepr;
			}
		}

		internal IList<QVertex> ShadowVertexRepr {
			get {
				return _shadowVertexRepr;
			}
		}
		...
		private Vector2 TransformPositionToViewport(Vector2 input) {
			RectangleF? v2 = this.Options.TransformToViewport;
			if (v2 == null) {
				return input;
			}
			RectangleF? v1 = ViewportHelper.CurrentViewport;
		...
		private static float TransformWidthToViewport(float input, QFontRenderOptions options) {
			RectangleF? v2 = options.TransformToViewport;
			if (v2 == null) {
				return input;
			}
			RectangleF? v1 = ViewportHelper.CurrentViewport;
		...
		private Vector3 TransformToViewport(Vector3 input) {
			Vector2 target = LockToPixel(TransformPositionToViewport(input.ToVector2()));
		...
		/// <returns></returns>
		private static float TextNodeLineLength(TextNode node, float maxLength) {
			if (node == null)
				return 0;
		...
		private static bool CrumbledWord(TextNode node) {
		...
		/// <returns></returns>
		private static bool SkipTrailingSpace(TextNode node, float lengthSoFar, float boundWidth) {
