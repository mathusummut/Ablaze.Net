	...
	class Builder {
		...
		private List<SizeF> GetGlyphSizes(Font font) {
			...
			for (int i = 0; i < charSet.Length; i++) {
				var charSize = graph.MeasureString(charSet[i].ToString(), font);
		...
		private static void RetargetGlyphRectangleInwards(BitmapData bitmapData, QFontGlyph glyph, bool setYOffset, byte alphaTolerance) {
			...
			if (bitmapData.PixelFormat == PixelFormat.Format32bppArgb)
				emptyPix = delegate (BitmapData data, int x, int y) {
					return QBitmap.EmptyAlphaPixel(data, x, y, alphaTolerance);
				};
			else
				emptyPix = delegate (BitmapData data, int x, int y) {
					return QBitmap.EmptyPixel(data, x, y);
				};
		...
		public QFontData BuildFontData(string saveName) {
			...
			const int margin = 2; //margin in initial bitmap (don't bother to make configurable - likely to cause confusion
		...
		public static void SaveQFontDataToFile(QFontData data, string filePath) {
			...
			using (StreamWriter writer = new StreamWriter(filePath + ".qfont")) {
		...
		public static QFontData LoadQFontDataFromFile(string filePath, float downSampleFactor, QFontConfiguration loaderConfig) {
			...
			using (FileStream file = new FileStream(filePath, FileMode.OpenOrCreate, FileAccess.Read, FileShare.ReadWrite)) {
			...
			string namePrefix = filePath.Replace(".qfont", "").Replace(" ", string.Empty);
