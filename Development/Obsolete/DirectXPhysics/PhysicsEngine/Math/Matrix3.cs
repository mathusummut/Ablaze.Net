...
namespace PhysicsEngine {
	...
	public struct Matrix3 : IEquatable<Matrix3> {
		...
		public Matrix3(ref Matrix matrix) {
			Vector4 row0 = matrix.get_Rows(0), row1 = matrix.get_Rows(1), row2 = matrix.get_Rows(2);
			Row0 = new Vector3(row0.X, row0.Y, row0.Z);
			Row1 = new Vector3(row1.X, row1.Y, row1.Z);
			Row2 = new Vector3(row2.X, row2.Y, row2.Z);
		}
		...
		public static void CreateFromQuaternion(ref Quaternion q, out Matrix3 result) {
			Quaternion quat = ToAxisAngle(ref q);
		...
		private static Quaternion ToAxisAngle(ref Quaternion quaternion) {
		...
		public static Vector3 Mult(ref Matrix3 matrix, ref Vector3 vertex) {
			return new Vector3(vertex.X * matrix.Row0.X + vertex.Y * matrix.Row0.Y + vertex.Z * matrix.Row0.Z,
				vertex.X * matrix.Row1.X + vertex.Y * matrix.Row1.Y + vertex.Z * matrix.Row1.Z,
				vertex.X * matrix.Row2.X + vertex.Y * matrix.Row2.Y + vertex.Z * matrix.Row2.Z);
		}
