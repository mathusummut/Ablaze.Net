...
namespace System.Text {
	...
	public interface IHjsonDsfProvider {
		...
		string Stringify(JsonObject value);
	}
	...
	public static class HjsonDsf {
		...
		public static IHjsonDsfProvider Hex(bool stringify) {
			return new DsfHex(stringify);
		}
		/// <summary>Returns a date DSF provider.</summary>
		public static IHjsonDsfProvider Date() {
			return new DsfDate();
		}
		...
		internal static JsonObject Parse(IEnumerable<IHjsonDsfProvider> dsfProviders, string value) {
		...
		internal static string Stringify(IEnumerable<IHjsonDsfProvider> dsfProviders, JsonObject value) {
			foreach (var dsf in dsfProviders) {
				try {
	...
	class DsfDate : IHjsonDsfProvider {
		...
		public string Description {
			get {
				return "support ISO dates";
			}
		}

		public JsonObject Parse(string text) {
			if (isDate.IsMatch(text) || isDateTime.IsMatch(text))
				return JsonObject.FromObject(DateTime.Parse(text));
			else
				return null;
		}
