...
namespace StarterKit {
	class Game : GraphicsForm {
		...
		private string _benchResult = string.Empty;
		...
		protected override void OnUpdate(double elapsedMilliseconds) {
			double increment = elapsedMilliseconds * 0.01;
			cnt += increment;

			if (boundsAnimationCnt < 1.0f)
				boundsAnimationCnt += increment * 0.2f;
			else
		...
		protected override void OnResize(EventArgs e) {
			...
			//_projectionMatrix = Matrix4.CreateOrthographicOffCenter(X, ViewSize.Width, Y, Height, -1.0f, 1.0f);
			_projectionMatrix = Matrix4.CreateOrthographicOffCenter(ClientRectangle.X, ViewSize.Width, ClientRectangle.Y, ClientRectangle.Height, -1.0f, 1.0f);
		...
		private void PrintComment(QFont font, string comment, QFontAlignment alignment, ref float yOffset, QFontRenderOptions opts) {
			...
			var dp = new QFontDrawingPrimitive(font, opts == null ? new QFontRenderOptions() : opts);
			dp.Print(comment, pos, new SizeF(ViewSize.Width - 60, -1), alignment);
			yOffset += dp.Measure(comment, new SizeF(ViewSize.Width - 60, -1), alignment).Height;
			drawing.DrawingPimitiveses.Add(dp);
		}
		...
		private void PrintCommentWithLine(QFont font, string comment, QFontAlignment alignment, float xOffset, ref float yOffset, QFontRenderOptions opts) {
			...
			dp.Print(comment, new Vector3(xOffset, Height - yOffset, 0f), new SizeF(ViewSize.Width - 60, -1), alignment);
			drawing.DrawingPimitiveses.Add(dp);
			var bounds = font.Measure(comment, new SizeF(ViewSize.Width - 60, float.MaxValue), alignment);
		...
		private void PrintCode(string code, ref float yOffset) {
			...
			drawing.Print(codeText, code, pos, new SizeF(ViewSize.Width - 50, -1), QFontAlignment.Left, codeTextOptions);
			yOffset += codeText.Measure(code, new SizeF(ViewSize.Width - 50, -1), QFontAlignment.Left).Height;
		}
		...
		protected override void OnPaintGL() {
			...
			//GL.Color3(0.9f, 0.9f, 0.9f); GL.Vertex2(ViewSize.Width, Height);
			//GL.Color3(0.9f, 0.9f, 0.9f); GL.Vertex2(ViewSize.Width, 0);
				...
				switch (currentDemoPage) {
					case 0: {
							yOffset += drawing.Print(heading1, "ModernQuickFont",
																	 new Vector3((float) ViewSize.Width / 2, Height, 0),
							...
							yOffset += drawing.Print(mainText, modernQuickFontIntro, new Vector3(30, Height - yOffset, 0),
																 new SizeF(ViewSize.Width - 60f, -1), QFontAlignment.Justify).Height;
					...
					case 1: {
							...
							drawing.Print(mainText, introduction, new Vector3(30, Height - yOffset, 0),
																 new SizeF(ViewSize.Width - 60f, -1), QFontAlignment.Justify);
					...
					case 3: {
							...
							PrintCommentWithLine(centredTextAsYou, QFontAlignment.Centre, ViewSize.Width * 0.5f, ref yOffset);
					...
					case 4: {
							...
							yOffset += drawing.Print(controlsText, "Press [Up], [Down] or [Enter]!",
																		 new Vector3(ViewSize.Width * 0.5f, Height - yOffset, 0f),
							...
							PrintWithBounds(mainText, ofCourseItsNot,
											new RectangleF(30f + boundShrink * 0.5f, yOffset, ViewSize.Width - 60 - boundShrink,
					...
					case 8: {
							...
							QFontRenderOptions monoSpaceCondensed = (QFontRenderOptions) monoSpacedOptions.Clone();
							...
							yOffset += 2f;
							PrintCommentWithLine(monoSpaced, mono, QFontAlignment.Centre, ViewSize.Width * 0.5f, ref yOffset, monoSpaceCondensed);
							yOffset += 2f;
					...
					case 9: {
							...
							yOffset += drawing.Print(mainText, nonPreProcessed,
																	 new Vector3(20, Height - yOffset, 0),
																	 new SizeF(ViewSize.Width - 40f, -1), QFontAlignment.Justify)
										.Height;
							_stopwatch.Stop();
							...
							drawing.Print(_benchmarkResults, _benchResult,
																		  new Vector3(ViewSize.Width * 0.5f, Height - yOffset, 0),
			...
			if (currentDemoPage != lastPage) {
				Vector3 pos = new Vector3(ViewSize.Width - 10 - 16 * (float) (1 + Math.Sin(cnt * 4)),
