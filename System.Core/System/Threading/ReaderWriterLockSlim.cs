	...
	public class ReaderWriterLockSlim : IDisposable {
		...
		private bool TryEnterReadLockCore(TimeoutTracker timeout) {
			...
			if (!fIsReentrant) {
				...
				if (id == writeLockOwnerId) {
					//Check for AW->AR
					throw new InvalidOperationException("LockRecursionException_ReadAfterWriteNotAllowed");
				}
				...
				if (lrwc.readercount > 0) {
					ExitMyLock();
					throw new InvalidOperationException("LockRecursionException_RecursiveReadWriteNotAllowed");
		...
		private bool TryEnterWriteLockCore(TimeoutTracker timeout) {
			...
			if (!fIsReentrant) {
				if (id == writeLockOwnerId) {
					//Check for AW->AW
					throw new InvalidOperationException("LockRecursionException_RecursiveWriteNotAllowed");
				} else if (id == upgradeLockOwnerId) {
				...
				if (lrwc != null && lrwc.readercount > 0) {
					ExitMyLock();
					throw new InvalidOperationException("LockRecursionException_WriteAfterReadNotAllowed");
				}
			} else {
				EnterMyLock();
				lrwc = GetThreadRWCount(false);
		...
		private bool TryEnterUpgradeableReadLockCore(TimeoutTracker timeout) {
			...
			if (!fIsReentrant) {
				if (id == upgradeLockOwnerId) {
					//Check for AU->AU
					throw new InvalidOperationException("LockRecursionException_RecursiveUpgradeNotAllowed");
				} else if (id == writeLockOwnerId) {
					//Check for AU->AW
					throw new InvalidOperationException("LockRecursionException_UpgradeAfterWriteNotAllowed");
				}
				...
				if (lrwc != null && lrwc.readercount > 0) {
					ExitMyLock();
					throw new InvalidOperationException("LockRecursionException_UpgradeAfterReadNotAllowed");
				}
			} else {
				EnterMyLock();
				lrwc = GetThreadRWCount(false);
